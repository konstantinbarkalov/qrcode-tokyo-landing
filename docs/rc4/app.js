// (template-ready for Next.js extraction)
...
(function () {
    ...
    const topFlyMeasured = qs('.fly--top .fly__measured');
    const bottomFlyMeasured = qs('.fly--bottom .fly__measured');

    function updateFlyHeight(el, varName) {
        const flyHeight = el.offsetHeight;
        setVar(varName, flyHeight + 'px');
        return flyHeight;
    }
    ...
    function updateTopFlyHeight() {
        topFlyHeight = updateFlyHeight(topFlyMeasured, '--space-header');
    }
    function updateBottomFlyHeight() {
        bottomFlyHeight = updateFlyHeight(bottomFlyMeasured, '--space-footer');
    }

    updateTopFlyHeight();
    updateBottomFlyHeight();

    new ResizeObserver(() => {
        updateTopFlyHeight();
    }).observe(topFlyMeasured);

    new ResizeObserver(() => {
        updateBottomFlyHeight();
    }).observe(bottomFlyMeasured);
    ...
    const parallax = (function () {
        ...
        function applyDeferred() {
            ...
            requestAnimationFrame(() => {
                ...
                const h = topFlyHeight;
                const t = -Math.min(y * factor, h + extraOffset);
                const nonlinear = -Math.pow(-t / identDist, gamma) * identDist;
                const integer = Math.round(nonlinear);
                if (integer !== lastInteger) {
                    topFly.style.transform = `translateY(${integer}px)`;
                    lastInteger = integer;
                }
                ticking = false;
            });
        }
        window.addEventListener('scroll', applyDeferred, { passive: true });
        applyDeferred();
        return { freeze: () => { active = false; }, resume: () => { if (!active) { active = true; applyDeferred(); } } };
    })();
    ...
    const dialogManager = (function () {
        ...
        function open(id, trigger) {
            const dlg = typeof id === 'string' ? qs(`#${id}`) : id; if (!dlg || dlg.open) return; if (openCount === 0) { lastFocus = document.activeElement; scrollLock.lock(); }
    ...
    (function () {
        ...
        const filterBar = qs('.ui-bar--filter', footer);
        const progressBar = qs('.ui-bar--progress', footer);

        const label = progressBar.querySelector('.ui-progress__label');
        const percent = progressBar.querySelector('.progress-percent');
        let progressRatio = 0;
        function setProgress(newRatio) {
            const clampedRatio = Math.max(0, Math.min(1, newRatio));
            progressBar.style.setProperty('--progress--ratio', clampedRatio);
            progressBar.setAttribute('aria-valuenow', String(Math.round(clampedRatio * 100)));
            percent.textContent = Math.round(clampedRatio * 100) + '%';
            const text = clampedRatio < 0.3 ? 'Generating…' : clampedRatio < 0.6 ? 'Still working…' : clampedRatio < 0.9 ? 'Almost done…' : 'Finalizing…';
            label.textContent = text;
            console.log(text);
            progressRatio = clampedRatio;
        }

        function animateTo(targetRatio, duration, labelDone) {
            const startRatio = progressRatio;
            const target = Math.max(0, Math.min(1, targetRatio));
            const delta = target - startRatio;
            ...
            function step(now) {
                ...
                else if (labelDone) label.textContent = labelDone;
            }
        ...
        function run() {
            ...
            progressBar.classList.remove('is-done', 'is-error');
            label.textContent = 'Generating…';
            animateTo(1.0, 2200, 'Done');
            setTimeout(() => {
        ...
        // Removed legacy demo half-progress button; explicit [data-progress-to] buttons provide targeted demos instead.

        // Delegated handlers for explicit progress targets
        document.addEventListener('click', (e) => {
            const btn = e.target.closest('[data-progress-to]');
            if (!btn) return;
            const val = parseInt(btn.getAttribute('data-progress-to') || '0', 10);
            const ratio = Math.max(0, Math.min(100, isNaN(val) ? 0 : val)) / 100;
            filterBar.hidden = true;
            progressBar.hidden = false;
            progressBar.classList.remove('is-done', 'is-error');
            label.textContent = 'Template demo…';
            animateTo(ratio, 700, ratio === 1 ? 'Done' : undefined);
            if (ratio === 1) {
                // Briefly show done state, then restore filter bar
                setTimeout(() => {
                    progressBar.classList.add('is-done');
                    setTimeout(() => {
                        progressBar.classList.remove('is-done');
    ...
    /* Nav current state handling */
    document.addEventListener('click', e => { const link = e.target.closest('[data-nav-item]'); if (!link) return; const nav = link.closest('.main-nav'); nav?.querySelectorAll('.is-current').forEach(el => el.classList.remove('is-current')); link.classList.add('is-current'); });

    /* Mode controls (art / blueprint) */
    const modeToggleInput = qs('.toggle-mockup .toggle__input');
    function syncModeUI() {
        if (modeToggleInput) modeToggleInput.checked = document.body.classList.contains('art-mode');
    }
    function setArtMode(enabled) {
        document.body.classList.toggle('art-mode', !!enabled);
        syncModeUI();
    }
    // Toggle mockup sync
    modeToggleInput?.addEventListener('change', (e) => {
        setArtMode(!!e.target.checked);
    });
    ...
    window.UI = { parallax, scrollLock, dialogs: dialogManager, toast };
})();